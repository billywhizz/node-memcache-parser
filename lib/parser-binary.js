			...
			switch(_parser.state) {
				case constants.parser.state.HEADER:
					if(loc == constants.general.MEMC_HEADER_LEN - 1) {
						...
						if(message.exlen > 0) {
							...
							if(!_parser.chunked) _body = new Buffer(message.totlen - (message.keylen + message.exlen));
						...
						loc = 0;
						toparse = (message.totlen - (message.exlen + message.keylen));
					}
					else {
						_header[loc++] = buffer[pos];
					}
				...
				case constants.parser.state.KEY:
					if(loc == message.keylen - 1) {
						...
						if(message.totlen - (message.keylen + message.exlen) > 0) {
							if(!_parser.chunked) _body = new Buffer(message.totlen - (message.keylen + message.exlen));
							_parser.state = constants.parser.state.BODY;
						}
						else {
				...
				case constants.parser.state.BODY:
					if(!_parser.chunked) {
						if(loc == message.totlen - (message.keylen + message.exlen) - 1) {
